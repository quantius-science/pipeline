function controller_view_averages_clean(job_settings, input_folder_name, output_folder_name)
    ...
    % get file with averaged results:
    objects_clean = load(fullfile(job_settings.path_job, 'objects_3_averaged_cleaned.mat'));
    objects_clean = objects_clean.objects;
    ...
    for i = 1:num_stacks
        ...
        objects_clean_stack = utilities.get_structure_results_matching_number(objects_clean, 'num_stack', list_stacks(i));
        ...
        for j = 1:num_slices
            ...
            objects_clean_slice = utilities.get_structure_results_matching_number(objects_clean_stack, 'num_slice', list_slices(j));
            
            % get number of objects:
            num_objects_slice = numel(objects_slice);
            num_objects_clean_slice = numel(objects_clean_slice);

            % for each object:
            for k = 1:num_objects_slice

                % add object to image:
                image_slice = utilities.add_result_to_image(job_settings.tool, image_slice, objects_slice(1,k).coordinates, 'yellow');

            end
            for k = 1:num_objects_clean_slice

                % add object to image:
                image_slice = utilities.add_result_to_image(job_settings.tool, image_slice, objects_clean_slice(1,k).coordinates, 'red');
